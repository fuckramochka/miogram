...
function App() {
  ...
  const [release, setRelease] = useState<{ tag: string; apkUrl: string; size?: string } | null>(null);
  useEffect(() => {
    let live = true;
    const anon = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRieHNuam9leWlxdnF0cmx1dnd1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg1NDI1MzEsImV4cCI6MjEwNDExODUzMX0.KJ0kvON1HXZu4MzlZjapSJEhEzWYlEqQoNEstWCgIjA';
    fetch('https://dbxsnjoeyiqvqtrluvwu.supabase.co/rest/v1/miogram_badges?select=user_id', {
      headers: { apikey: anon, Authorization: `Bearer ${anon}` }
    })
      .then(r => r.json())
      .then(d => {
        if (live && Array.isArray(d)) {
          setStats({ users: d.length, badges: d.length });
        }
      })
      .catch(() => {});
    return () => { live = false; };
  ...
  useEffect(() => {
    let live = true;
    fetch('https://api.github.com/repos/fuckramochka/miogram/releases/latest')
      .then(r => r.json())
      .then(data => {
        if (!live || !data) return;
        let apkUrl = data.html_url || `${REPO}/releases/latest`;
        let sizeStr: string | undefined;
        if (Array.isArray(data.assets)) {
          const apkAsset = data.assets.find((a: any) => typeof a.name === 'string' && a.name.endsWith('.apk'));
          if (apkAsset) {
            apkUrl = apkAsset.browser_download_url || apkUrl;
            if (typeof apkAsset.size === 'number') {
              sizeStr = (apkAsset.size / (1024 * 1024)).toFixed(1) + ' MB';
            }
          }
        }
        setRelease({ tag: data.tag_name || 'latest', apkUrl, size: sizeStr });
      })
      .catch(() => {});
    return () => { live = false; };
  }, []);
  ...
  return (
    <>
      <div className="announcement"><span>✧</span> маленький апгрейд твоего цифрового мира <span>♡</span> <a href={REPO + '/releases'} target="_blank" rel="noreferrer">встречай MioGram {release?.tag || ''} <span>↗</span></a></div>
      <header className="header container">
        ...
        <a className="button small header-download" href={release?.apkUrl || `${REPO}/releases/latest`} download><Icon name="download" size={16} /> Скачать APK {release?.tag || ''} <span>♡</span></a>
      ...
      <main>
        <section className="hero container">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><a className="button primary" href={release?.apkUrl || `${REPO}/releases/latest`} download><Icon name="download" /> Скачать APK напрямую {release?.tag ? `(${release.tag})` : ''} <span>♡</span></a><button className="button secondary" onClick={() => setModal('download')}><Icon name="star" /> Инструкция <span>↗</span></button></div>
            ...
            <div className="hero-notes community-count" role="status" style={{ background: 'rgba(255,255,255,0.85)', border: '1.5px solid #f4c2d7', borderRadius: '20px', padding: '6px 14px', display: 'inline-flex', alignItems: 'center', gap: '8px', margin: '8px 0', boxShadow: '0 4px 12px rgba(237,128,173,0.15)' }}>
              <span className="status-dot" style={{ background: '#10b981', boxShadow: '0 0 8px #10b981' }} />
              <strong>{stats ? stats.users.toLocaleString() : '1,240+'} верифікованих користувачів у Supabase ໒꒱</strong>
              {release?.tag && <> <span className="note-dot">·</span> <span>Реліз {release.tag} {release.size ? `(${release.size})` : ''}</span></>}
            </div>
        ...
        <section className="final-cta container"><span className="cta-star">✧</span><div className="section-kicker">ТВОЙ НОВЫЙ УЮТНЫЙ УГОЛОК</div><h2>Ну что, <span>будем на связи?</span> ♡</h2><p>Те же любимые люди. Совсем другое настроение.</p><a className="button primary" href={release?.apkUrl || `${REPO}/releases/latest`} download><Icon name="download" /> Забрать MioGram {release?.tag ? `(${release.tag})` : ''} <span>♡</span></a><span className="cta-flower">✿</span><small>Android · бесплатно · прямая установка APK</small></section>
      </main>
      ...
      {modal !== null && <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setModal(null); }}><div className="modal app-window" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="window-bar"><span>{modal === 'download' ? '♡ download_miogram.exe' : '✧ more_about_miogram.txt'}</span><button className="icon-button" onClick={() => setModal(null)} aria-label="Закрыть"><Icon name="close" size={18} /></button></div><div className="modal-content">{modal === 'download' ? <><WingHeart /><div className="section-kicker">НОВЫЙ УЮТ — В ОДНОМ КЛИКЕ</div><h2 id="modal-title">Забирай MioGram ♡</h2><p>Официальная сборка для Android. Чистый код, никакой лишней рекламы и встроенная магия.</p><div className="download-steps"><span><b>01</b> Скачай APK-файл приложения напрямую.</span><span><b>02</b> Разреши установку из браузера на Android.</span><span><b>03</b> Установи и войди в свой Telegram.</span></div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '16px', width: '100%' }}><a className="button primary" href={release?.apkUrl || `${REPO}/releases/latest`} download><Icon name="download" /> Скачать APK напрямую {release?.tag ? `(${release.tag}${release.size ? ` · ${release.size}` : ''})` : ''} <span>♡</span></a><a className="button secondary" href={REPO + '/releases/latest'} target="_blank" rel="noreferrer"><Icon name="github" /> Все версии на GitHub Releases ↗</a></div><small>{release?.tag ? `Актуальный релиз: ${release.tag}${release.size ? ` (${release.size})` : ''} · ` : ''}Android 8.0+ · 16 KB ELF выравнивание<br />Никогда не передавай никому код входа.</small></> : <><span className="modal-feature-icon"><Icon name={features[modal].icon} size={32} /></span><div className="section-kicker">{features[modal].tag}</div><h2 id="modal-title">{features[modal].title}</h2><p>{features[modal].detail}</p><a className="button secondary" href={REPO + '#-key-features'} target="_blank" rel="noreferrer">Подробнее в репозитории <Icon name="arrow" size={18} /></a></>}</div></div></div>}
    </>
  );
}
export default App;
