  ...
  return (
    <>
      ...
      <header className="header container">
        ...
        <button className="button small header-download" onClick={() => setModal('download')}><Icon name="download" size={16} /> Скачать {release?.tag || ''} <span>♡</span></button>
      ...
      <main>
        <section className="hero container">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><button className="button primary" onClick={() => setModal('download')}><Icon name="download" /> Скачать MioGram {release?.tag ? `(${release.tag})` : ''} <span>♡</span></button><a className="button secondary" href={REPO} target="_blank" rel="noreferrer"><Icon name="github" /> Исходный код <span>↗</span></a></div>
            ...
            {stats && <div className="hero-notes community-count" role="status"><span className="status-dot" /><span>{stats.users.toLocaleString()} пользователей в Supabase</span>{release?.tag && <> <span className="note-dot">·</span> <span>Релиз {release.tag} {release.size ? `(${release.size})` : ''}</span></>}</div>}
